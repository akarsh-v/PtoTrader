												...
												<?php $i=1; foreach($tbody as $t){ ?>
												  <tr>
													...
													<td><?php if($t['active']==1){ $check='checked'; }else{ $check = ''; } ?>
															<label class="switch">
															  <input onclick="active('/admin/<?=controller; ?>/active/<?=$t['api_id']; ?>/<?=$t['active']; ?>');" type="checkbox" <?=$check ?> value="<?=$t['active']; ?>">
															  <div class="slider round"></div>
															</label>
														</td>
														<td style="width: 14%;">
														    <a onclick="getform('/admin/<?=controller; ?>/click/update/<?=$t['api_id']; ?>');" class="btn btn-default btn-icon-anim btn-circle"><i class="fa fa-pencil"></i></a>
														</td>
												  </tr>
												<?php $i++; } ?>
												</tbody>
